...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!passwordOk) {
      ...
      const blockUntil =
        failedAttempts >= 3 ? new Date(Date.now() + 30 * 1000) : null;
      await user.update({ failedAttempts, blockUntil });
      await logEvent(
        username,
        "LOGIN",
        failedAttempts >= 3 ? "TEMP_BLOCKED" : "FAILED_PASSWORD",
      );
      return res.status(401).json({ message: "Credenciales incorrectas" });
    }
...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (payload.purpose !== "password-reset") {
      return res
        .status(401)
        .json({ message: "Token de recuperacion no valido" });
    }
    ...
    if (!user || !user.passwordResetRequired) {
      return res
        .status(404)
        .json({ message: "Solicitud de recuperacion no encontrada" });
    }
